...
# Supernova toggle: if this file exists, boot the multithreaded server with the
# thread count it contains (see wr-boot.scd). Absent = scsynth (default).
[ -f "$WR/supernova.threads" ] && export ATELIER_THREADS="$(cat "$WR/supernova.threads")"
